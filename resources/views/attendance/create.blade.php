	...
	<div class="flex h-full w-full flex-1 flex-col gap-6">
		<div class="flex items-center gap-4">
			<flux:button
				...
				wire:navigate
			/>
			<flux:heading size="xl">{{ __('Add Attendance') }}</flux:heading>
		</div>
		...
		<flux:card class="flex-1">
			<form method="POST" action="{{ route('attendances.store') }}" class="flex h-full flex-col gap-6">
				@csrf
				...
				<div class="grid flex-1 content-start auto-rows-min gap-6 sm:grid-cols-2">
					...
					<flux:field>
						...
						<flux:input type="date" name="attendance_date" value="{{ old('attendance_date', now()->format('Y-m-d')) }}" required />
						<flux:error name="attendance_date" />
					</flux:field>

					<flux:field>
						<flux:label>{{ __('Work In') }}</flux:label>
						<flux:input type="time" name="work_in" value="{{ old('work_in') }}" required />

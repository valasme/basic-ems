	...
	<main class="flex h-full w-full flex-1 flex-col gap-6" role="main" aria-labelledby="page-title">
		<div class="flex items-center gap-4">
			<flux:button
				...
				wire:navigate
			/>
			<flux:heading id="page-title" size="xl">{{ __('Add Note') }}</flux:heading>
		</div>
		...
		<flux:card class="flex-1">
			<form method="POST" action="{{ route('notes.store') }}" class="flex h-full flex-col gap-6" aria-describedby="note-form-help">
				@csrf

				<p id="note-form-help" class="sr-only">
					{{ __('Required fields must be completed before creating the note.') }}
				</p>

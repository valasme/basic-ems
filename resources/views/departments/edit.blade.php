<x-layouts::app :title="__('Edit :name - BasicEMS', ['name' => $department->name])">
	<main class="flex h-full w-full flex-1 flex-col gap-6" role="main" aria-labelledby="page-title">
		<div class="flex items-center gap-4">
			<flux:button
				...
				wire:navigate
			/>
			<flux:heading id="page-title" size="xl">{{ __('Edit Department') }}</flux:heading>
		</div>
		...
		<flux:card class="flex-1">
			<form method="POST" action="{{ route('departments.update', $department) }}" class="flex h-full flex-col gap-6" aria-describedby="department-form-help">
				@csrf
				@method('PUT')

				<p id="department-form-help" class="sr-only">
					{{ __('Department name is required before updating the department.') }}
				</p>
				...
				<fieldset class="grid flex-1 content-start auto-rows-min gap-6">
					...
					<flux:field>
						...
						>{{ old('description', $department->description) }}</flux:textarea>

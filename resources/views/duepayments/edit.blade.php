<x-layouts::app :title="__('Edit Due Payment - BasicEMS')">
	<div class="flex h-full w-full flex-1 flex-col gap-6">
		<div class="flex items-center gap-4">
			<flux:button
				...
				wire:navigate
			/>
			<flux:heading size="xl">{{ __('Edit Due Payment') }}</flux:heading>
		</div>
		...
		<flux:card class="flex-1">
			...
			<form
				method="POST"
				action="{{ route('due-payments.update', $duePayment) }}"
				class="flex h-full flex-col gap-6"
				x-data="{
					selectedEmployee: '{{ old('employee_id', $duePayment->employee_id) }}',
				...
				<div class="grid flex-1 content-start auto-rows-min gap-6 sm:grid-cols-2">
					<flux:field class="sm:col-span-2">
						...
						<flux:select name="employee_id" x-model="selectedEmployee" required>
							...
							@foreach ($employees as $employee)
								<option value="{{ $employee->id }}" @selected(old('employee_id', $duePayment->employee_id) == $employee->id)>
									{{ $employee->full_name }}
									@if ($employee->pay_day)
					...
					<flux:field>
						...
						<flux:input
							type="number"
							name="amount"
							value="{{ old('amount', $duePayment->amount) }}"
					...
					<flux:field>
						...
						<flux:input
							type="date"
							name="pay_date"
							value="{{ old('pay_date', $duePayment->pay_date->format('Y-m-d')) }}"
							required
						/>
					...
					<flux:field>
						...
						<flux:select name="status" required>
							@foreach ($statuses as $value => $label)
								<option value="{{ $value }}" @selected(old('status', $duePayment->status) === $value)>
									{{ $label }}
								</option>
							@endforeach
						</flux:select>
					...
					<flux:field class="sm:col-span-2">
						...
						>{{ old('notes', $duePayment->notes) }}</flux:textarea>
